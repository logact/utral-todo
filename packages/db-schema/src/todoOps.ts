import { eq, and, or, lt, gte, isNotNull, isNull, not } from 'drizzle-orm';
import {
  todos,
  todoRelations,
  plans as plansTable,
...
import { createPlan } from './planOps.js';
...
export async function createTodo(
  ...
  await store.db.insert(todos).values(newRow);
  await store.notifyDbOperation('todos', 'create', todo.id);
...
export async function createGoal(
  ...
  const goal = await createTodo(store, title, {
    nodeType: 'goal',
    ...options,
  });

  const plan = await createPlan(store, goal.id, 'Default Plan');
  await updateTodo(store, goal.id, { activePlanId: plan.id });
...
export async function createTask(
  ...
  return createTodo(store, title, {
    nodeType: 'task',
    ...options,
  });
}

export async function getAllTodos(store: DbStore): Promise<Todo[]> {
...
export async function getRootTodos(store: DbStore): Promise<Todo[]> {
  const rows = (await store.db
    .select()
    .from(todos)
    .where(and(eq(todos.isDeleted, false), isNull(todos.parentId)))) as any[];
  return rows.map(rowToTodo);
}

export async function getRootGoal(store: DbStore): Promise<Todo | undefined> {
  const rows = (await store.db.select().from(todos).where(eq(todos.isRootGoal, true))) as any[];
  const row = rows[0];
  return row ? rowToTodo(row) : undefined;
}

export async function getSubTodos(store: DbStore, parentId: string): Promise<Todo[]> {
  const rows = (await store.db
    .select()
    .from(todos)
    .where(and(eq(todos.parentId, parentId), eq(todos.isDeleted, false)))) as any[];
...
export async function getSubGoals(store: DbStore, parentId: string): Promise<Todo[]> {
  const rows = (await store.db
    .select()
    .from(todos)
    .where(
...
export async function traceParentChain(store: DbStore, todoId: string): Promise<Todo[]> {
  const chain: Todo[] = [];
  ...
  while (currentId && !visited.has(currentId)) {
    ...
    const todo = await getTodo(store, currentId);
    if (!todo) break;
    chain.unshift(todo);
...
export async function traceGoalChain(store: DbStore, todoId: string): Promise<Todo[]> {
  const chain = await traceParentChain(store, todoId);
  return chain.filter((t) => t.nodeType === 'goal');
}

export async function getTodoDescendants(store: DbStore, todoId: string): Promise<Todo[]> {
  const descendants: Todo[] = [];

  async function collect(parentId: string) {
    const children = await getSubTodos(store, parentId);
...
export async function reorderSubTodos(
  store: DbStore,
  _parentId: string,
  orderedIds: string[]
): Promise<void> {
  ...
  for (let i = 0; i < orderedIds.length; i++) {
    const rows = (await store.db
      .select()
      .from(todos)
      .where(eq(todos.id, orderedIds[i]))) as any[];
    ...
    await store.notifyDbOperation('todos', 'update', orderedIds[i]);
  }
}

export async function reorderTodos(store: DbStore, orderedIds: string[]): Promise<void> {
  const nodeId = store.deviceId;
  for (let i = 0; i < orderedIds.length; i++) {
    const rows = (await store.db
      .select()
...
export async function getTodo(store: DbStore, id: string): Promise<Todo | undefined> {
  const rows = (await store.db.select().from(todos).where(eq(todos.id, id))) as any[];
  const row = rows[0];
  return row ? rowToTodo(row) : undefined;
}
...
export async function updateTodo(
  ...
  const existing = await getTodo(store, id);
  ...
  await store.notifyDbOperation('todos', 'update', id);
}

export async function deleteTodo(store: DbStore, id: string): Promise<void> {
  const todo = await getTodo(store, id);
  if (todo?.isRootGoal) {
  ...
  if (todo?.nodeType === 'goal') {
    const planRows = (await store.db
      .select()
      .from(plansTable)
      .where(eq(plansTable.goalTodoId, id))) as any[];
    ...
    for (const plan of goalPlans) {
      ...
      await store.notifyDbOperation('plans', 'delete', plan.id);
    }
  } else {
    const allPlanRows = (await store.db.select().from(plansTable)) as any[];
    const allPlans = allPlanRows.map(rowToPlan);
    const allEdgeRows = (await store.db.select().from(actionEdges)) as any[];
    const edges = allEdgeRows.map(rowToActionEdge);
    for (const plan of allPlans) {
      if (plan.nodeIds.includes(id)) {
        ...
        await store.db
          ...
          .where(eq(plansTable.id, plan.id));
        await store.notifyDbOperation('plans', 'update', plan.id);
      }
    }
  }
  ...
  await store.db
    ...
    .where(eq(todos.id, id));
  await store.notifyDbOperation('todos', 'delete', id);
}
...
export async function getVirtualTodosForDate(store: DbStore, date: Date): Promise<Todo[]> {
  const d = new Date(date);
  d.setHours(0, 0, 0, 0);

  const templates = await getRepeatTemplates(store);
  if (templates.length === 0) return [];

  const templateIds = templates.map((t) => t.id);
  const allOccRows = (await store.db.select().from(repeatOccurrences)) as any[];
  const allOccurrences = allOccRows.map(rowToRepeatOccurrence);
...
export async function getTodaysTodos(store: DbStore): Promise<Todo[]> {
  ...
  const virtualTodos = await getVirtualTodosForDate(store, today);
...
export async function getTodosForDate(store: DbStore, date: Date): Promise<Todo[]> {
  ...
  const realTodos = rows.map(rowToTodo);
  const virtualTodos = await getVirtualTodosForDate(store, date);

  return [...realTodos, ...virtualTodos];
}

export async function getUnscheduledTodos(store: DbStore): Promise<Todo[]> {
  const rows = (await store.db
    .select()
    .from(todos)
    .where(
      and(
        isNull(todos.scheduledDate),
        not(eq(todos.status, 'done')),
        eq(todos.nodeType, 'task'),
        eq(todos.isDeleted, false),
        or(isNull(todos.isSystemTask), eq(todos.isSystemTask, false))
      )
...
export async function getOverdueTodos(store: DbStore): Promise<Todo[]> {
  const now = new Date();
...
export async function getInProgressTodos(store: DbStore): Promise<Todo[]> {
  const rows = (await store.db
    .select()
    .from(todos)
    .where(
...
export async function getUnscheduledHighPriorityTodos(store: DbStore): Promise<Todo[]> {
  const rows = (await store.db
    .select()
    .from(todos)
    .where(
...
export async function getTodaysGoals(store: DbStore): Promise<Todo[]> {
  const today = new Date();
  today.setHours(0, 0, 0, 0);
  const tomorrow = new Date(today);
  tomorrow.setDate(tomorrow.getDate() + 1);
...
export async function getTodosByTag(store: DbStore, tag: string): Promise<Todo[]> {
  const rows = (await store.db
    .select()
    .from(todos)
...
export async function getRepeatTemplates(store: DbStore): Promise<Todo[]> {
  const rows = (await store.db
    .select()
    .from(todos)
    .where(
