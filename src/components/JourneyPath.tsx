...
function layoutGraph(
  ...
  // For layout, we traverse from goal downward (reverse edges)
  ...
  for (const n of nodes) {
    ...
    parentOf.set(n.todo.id, []);
  }
  for (const edge of edges) {
    if (!nodeIds.has(edge.fromTodoId) || !nodeIds.has(edge.toTodoId)) continue;
    // edge goes from -> to; in our model, from is lower, to is upper (toward goal)
    // So from is a "child" of to in the tree sense
  ...
  // Nodes with no path from goal get placed at bottom
  ...
  // Sort within each depth: by status (pending first), then title
...
export function JourneyPath({ goalTodo, edges, highlightTodoId, onCreateEdge, onDeleteEdge, onEdgesChange, onNodeClick }: JourneyPathProps) {
  ...
  const loadGraph = useCallback(async () => {
    ...
    // Find all todos connected to the goal via action edges
    ...
    // Only show todos explicitly connected via action edges
  ...
  // Force SVG recompute after layout
  useEffect(() => {
    const timer = setTimeout(() => setSvgKey(k => k + 1), 150);
    return () => clearTimeout(timer);
  }, [graphNodes]);

  // Compute SVG edge positions
  const edgePositions = useMemo(() => {
    ...
    for (const edge of edges) {
      const fromEl = nodeRefs.current.get(edge.fromTodoId);
      const toEl = nodeRefs.current.get(edge.toTodoId);
      ...
      positions.push({
        edge,
        fromX: fromRect.left - containerRect.left + fromW / 2,
        fromY: fromRect.top - containerRect.top,
        toX: toRect.left - containerRect.left + (edge.toTodoId === goalTodo.id ? GOAL_W : NODE_W) / 2,
        toY: toRect.bottom - containerRect.top,
      });
    }
    return positions;
  }, [edges, goalTodo.id, svgKey]);
  ...
  async function handleAddNode() {
    ...
    // When on a no-goal node, connect to the current (highlighted) node;
    // otherwise connect to the goal
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
      ...
      {/* Graph */}
      <div ref={containerRef} className="relative overflow-x-auto py-6" style={{ minHeight: dims.h }}>
        {/* SVG edges */}
        <svg key={svgKey} className="absolute inset-0 pointer-events-none"
          style={{ width: dims.w, height: dims.h, minWidth: '100%' }}>
          <defs>
            ...
            return (
              <g key={pos.edge.id} opacity={isDimmed ? 0.08 : 1}>
                <path d={`M ${pos.fromX} ${pos.fromY} Q ${ctrlX} ${ctrlY} ${pos.toX} ${pos.toY}`}
                  ...
                  markerEnd={`url(#arrow-${pos.edge.type})`}
        ...
        {graphNodes.map(node => {
          ...
          return (
            <div key={node.todo.id}
              ref={el => { if (el) nodeRefs.current.set(node.todo.id, el); }}
              className="absolute"
              style={{
