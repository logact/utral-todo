    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="text-[10px] text-slate-400 dark:text-slate-500">
          {calcTotalMinutes(intervals, repeatCount)} min total
        </span>
      </div>
...
function PluseCard({
  ...
  onUpdate: (id: string, updates: Partial<Pick<Pluse, 'name' | 'description' | 'intervals' | 'repeatCount'>>) => void;
}) {
...
export function PluseList() {
  ...
  const [newIntervals, setNewIntervals] = useState<number[]>([25, 5]);
  const [newRepeatCount, setNewRepeatCount] = useState(1);

  const loadPluses = useCallback(async () => {
  ...
  async function handleCreate() {
    ...
    await createPluse(newName.trim(), [...newIntervals], newRepeatCount, newDesc.trim());
    setNewName('');
    setNewDesc('');
    setNewIntervals([25, 5]);
    setNewRepeatCount(1);
    setIsCreating(false);
    await loadPluses();
  }
